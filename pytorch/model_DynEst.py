"""
Dynest AudioCNN ported from the Bark project.

Original architecture: https://github.com/LiDCC/GuzhengTech99 (ICASSP 2023)
This copy adapts the encoder so it can be trained inside the HPT velocity
pipeline (single audio input, sigmoid velocity head).
"""
...
class DynestAudioCNN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.feature_extractor, self.freq_bins = get_feature_extractor_and_bins(
            audio_feature, sr, fft_size, fps
        )
        self.bn0 = nn.BatchNorm2d(self.freq_bins, momentum=0.01)
        self.adapt_conv = None  # lazy 1x1 channel mapper

        # Allow overriding encoder strides through optional cfg nodes.
        temporal_cfg = getattr(cfg, "dynest", None) or getattr(cfg, "cnn", None) or cfg.model
        temporal_strides = getattr(
            temporal_cfg, "temporal_scale", getattr(temporal_cfg, "temporal_strides", None)
        )

        self.encoder = SYMultiScaleAttnFlex(
            self.freq_bins, classes_num, temporal_strides=temporal_strides
        )

    def _ensure_channel_dim(self, x: torch.Tensor) -> torch.Tensor:
        """Match feature channels to the expected freq bin count."""
        ...
        if (
            ...
            self.adapt_conv = nn.Conv1d(n_channels, self.freq_bins, kernel_size=1, bias=False).to(
                x.device
            )
